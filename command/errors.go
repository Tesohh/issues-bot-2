...
var (
	...
	ErrNotInIssueThreadAndNoCode    = errors.New("issue not found, as command wasn't executed in an issue's thread and `code` was not specified")
	ErrNotInIssueThread             = errors.New("issue not found, as command wasn't executed in an issue's thread")
	ErrWrongRole                    = errors.New("role is invalid")
	ErrManPageDoesNotExist          = errors.New("man page does not exist")
	ErrCannotAddSelfAsDependency    = errors.New("cannot this issue to itself as a dependency")
	ErrIssueIsBlockedByDependencies = errors.New("this issue is blocked by open dependencies, and cannot be marked as done")
)
